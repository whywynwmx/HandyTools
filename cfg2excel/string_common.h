 ...
 */
#define CGMISC_CONVERT_VARGS(_dest,_format,_size) \
char _cstring[_size]; \
va_list _args; \
...
{
	std::string Result;
	CGMISC_CONVERT_VARGS(Result, format, CGMISC::MaxCStringSize);
	return Result;
}
...
inline std::string toString(const uint64 &val) { return toString("%llu", val); }
inline std::string toString(const sint64 &val) { return toString("%lld", val); }
...
#if (SIZEOF_SIZE_T) == 8
inline std::string toString(const size_t &val) { return toString("%llu", val); }
#else
#ifdef NL_OS_MAC
...
inline bool fromString(const std::string &str, uint64 &val) { bool ret = sscanf(str.c_str(), "%llu", &val) == 1; if (!ret) val = 0; return ret; }
inline bool fromString(const std::string &str, sint64 &val) { bool ret = sscanf(str.c_str(), "%lld", &val) == 1; if (!ret) val = 0; return ret; }
